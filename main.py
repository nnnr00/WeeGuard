...
def init_db():
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS users_v3 (user_id BIGINT PRIMARY KEY, points INTEGER DEFAULT 0, last_checkin_date DATE, checkin_count INTEGER DEFAULT 0, verify_fails INTEGER DEFAULT 0, verify_lock TIMESTAMP, verify_done BOOLEAN DEFAULT FALSE, wx_fails INTEGER DEFAULT 0, wx_lock TIMESTAMP, wx_done BOOLEAN DEFAULT FALSE, ali_fails INTEGER DEFAULT 0, ali_lock TIMESTAMP, ali_done BOOLEAN DEFAULT FALSE, username TEXT, vip_expire TIMESTAMP, daily_free_count INTEGER DEFAULT 0, last_free_date DATE, vip_buy_fails INTEGER DEFAULT 0, vip_buy_lock TIMESTAMP, verify_unlock_date DATE);")
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS system_keys_v7 (id INTEGER PRIMARY KEY, key_1 TEXT, link_1 TEXT, key_2 TEXT, link_2 TEXT, key_3 TEXT, link_3 TEXT, key_4 TEXT, link_4 TEXT, key_5 TEXT, link_5 TEXT, key_6 TEXT, link_6 TEXT, key_7 TEXT, link_7 TEXT, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);")
...
def get_session_date():
    ...
    return (now - timedelta(days=1)).date() if now.hour < 10 else now.date()
...
async def weekly_reset_task():
    ...
    if bot_app and ADMIN_ID:
        try: await bot_app.bot.send_message(ADMIN_ID, msg, parse_mode='Markdown')
        except: pass
...
async def delete_messages_task(chat_id, message_ids):
    try:
        ...
        for msg_id in message_ids:
            try: await bot_app.bot.delete_message(chat_id=chat_id, message_id=msg_id)
            except: pass
        text = "⏳ **消息存在时间有限，已自动销毁。**\n\n请到购买处重新获取（已购买不需要二次付费）。"
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("🎁 前往兑换中心", callback_data="go_exchange")], [InlineKeyboardButton("🏠 返回首页", callback_data="back_to_home")]])
...
async def noop_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if "vip_bought" in query.data: await query.answer("✅ 无需重复购买", show_alert=True)
    ...
    else: await query.answer("⚠️ 此处为空", show_alert=True)
...
async def get_quark_key_entry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    row = get_system_keys_v7()
    if not row: await query.message.reply_text("⏳ 系统初始化中，请稍后再试。"); return
    kb = []
    row1 = []
    for i in range(1, 3):
    ...
    await query.edit_message_text("🔑 **免费获取解锁密钥**\n\n1. 点击按钮跳转\n2. 保存文件，文件名为密钥\n3. 发送给机器人解锁兑换\n⚠️ 7天内每个密钥限用一次。", reply_markup=InlineKeyboardMarkup(kb), parse_mode='Markdown')
...
async def check_start_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt.startswith("20260"):
        ...
        gl = get_group_link()
        await update.message.reply_text("✅ **验证成功！**\n您已成功加入会员群。", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("👉 点击加入会员群", url=gl)]]), parse_mode='Markdown')
        await asyncio.sleep(2); await start(update, context); return ConversationHandler.END
    else:
...
async def check_recharge_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if valid:
        update_points(user_id, 100, "充值"); mark_success(user_id, pt)
        await update.message.reply_text("✅ **已充值 100 积分**", parse_mode='Markdown'); await asyncio.sleep(1); await jf_command_handler(update, context); return ConversationHandler.END
    else:
        fails, _, _ = check_lock(user_id, pt); new_fails = update_fail(user_id, pt, fails, 3 * 60)
        if new_fails >= 2: await update.message.reply_text("❌ **失败 (2/2)**\n⚠️ 此渠道锁定 3 小时。", parse_mode='Markdown'); await jf_command_handler(update, context); return ConversationHandler.END
...
async def buy_vip_card(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_v: await query.message.reply_text("✅ 您已是终身会员，无需重复购买！"); return ConversationHandler.END
...
async def ask_vip_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer(); fid = get_file_id("ALI_ORDER_TUTORIAL")
    text = "📝 **验证步骤：**\n请复制 **商家订单号**\n👇 **请在下方输入订单号：**"
    if fid: 
        try: await query.message.reply_photo(fid, caption=text, parse_mode='Markdown')
        except: await query.message.reply_text(text, parse_mode='Markdown')
...
async def check_vip_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt.startswith("4768"):
        ...
        else: await update.message.reply_text(f"❌ **订单号错误**\n剩余机会：{2 - new_fails}次", parse_mode='Markdown'); return WAITING_VIP_ORDER
...
async def dh_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_unlocked:
        ...
        if update.callback_query: await update.callback_query.answer("🔒 请先解锁！", show_alert=True); return
        else: await update.message.reply_text("🔒 **兑换中心已锁定**\n请先在积分中心获取密钥解锁！", reply_markup=kb, parse_mode='Markdown'); return
    ...
    is_v, _ = is_vip(user_id); daily_used, has_free = check_daily_free(user_id)
    kb = []; kb.append([InlineKeyboardButton("🎁 测试商品 (0积分)", callback_data="confirm_buy_test")])
    for r in rows:
        is_bought = check_purchase(user_id, r[0])
        btn_text = f"✅ {r[1]} (已兑换)" if is_bought else f"🎁 {r[1]} ({'免费(会员)' if is_v and has_free else str(r[2])+'积分'})"
...
async def exchange_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "confirm_buy_test": await query.edit_message_text("❓ 确认兑换测试商品？", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ 确认", callback_data="do_buy_test"), InlineKeyboardButton("❌ 取消", callback_data="list_prod_0")]])); return
    elif data == "do_buy_test": await query.edit_message_text("🎉 兑换成功！内容：哈哈", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 返回", callback_data="list_prod_0")]])); return
    ...
    if "confirm_buy_" in data:
        ...
        is_v, _ = is_vip(uid); _, has_free = check_daily_free(uid); cost_text = "0 积分 (会员特权)" if is_v and has_free else f"{prod[2]} 积分"
        await query.edit_message_text(f"❓ **确认兑换**\n商品：{prod[1]}\n价格：{cost_text}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ 确认支付", callback_data=f"do_buy_{pid}"), InlineKeyboardButton("❌ 取消", callback_data="list_prod_0")]]), parse_mode='Markdown'); return
    if "do_buy_" in data:
        prod = get_product_details(pid)
        if not prod: await query.answer("商品已下架", show_alert=True); return
        is_v, _ = is_vip(uid); _, has_free = check_daily_free(uid); price = prod[2]
        if is_v and has_free: use_free_chance(uid)
        else:
            if get_user_data(uid)[0] < price: await query.edit_message_text("❌ **余额不足！**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 返回", callback_data="list_prod_0")]])); return
        ...
        await asyncio.sleep(1); await dh_command(update, context)

# --- Admin Handlers (必须在这里定义) ---
...
async def admin_entry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.callback_query: await update.callback_query.edit_message_text("⚙️ **管理员后台**", reply_markup=kb, parse_mode='Markdown')
    else: await update.message.reply_text("⚙️ **管理员后台**", reply_markup=kb, parse_mode='Markdown')
    return ConversationHandler.END
...
# Admin Sub-Functions (Products, Cmds, Files, Keys)
async def manage_products_entry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("➕ 上架新商品", callback_data="add_product_start")], [InlineKeyboardButton("📂 管理/下架商品", callback_data="list_admin_prods_0")], [InlineKeyboardButton("🔙 返回后台", callback_data="back_to_admin")]])
...
async def list_admin_prods(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rows, total = get_products_list(limit=10, offset=offset); kb = []
...
async def confirm_del_prod(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer(); pid = int(query.data.split("_")[-1]); delete_product(pid)
...
async def manage_cmds_entry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("➕ 添加新命令", callback_data="add_new_cmd")], [InlineKeyboardButton("📂 管理/删除命令", callback_data="list_cmds_0")], [InlineKeyboardButton("🔙 返回后台", callback_data="back_to_admin")]])
...
async def confirm_del_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer(); cmd_id = int(query.data.split('_')[-1]); delete_command_by_id(cmd_id)
    await query.edit_message_text("🗑 **已删除。**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 返回列表", callback_data="list_cmds_0")]]), parse_mode='Markdown')
...
async def receive_cmd_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ `{name}` 创建。\n👇 发送内容 (多条)，完成后点按钮。", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ 完成", callback_data="finish_cmd_bind")]]), parse_mode='Markdown'); return WAITING_CMD_CONTENT
async def receive_cmd_content(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = update.message; cid = context.user_data.get('ccd'); fid = None; ftype = 'text'; txt = msg.text or msg.caption
    if msg.photo: fid=msg.photo[-1].file_id; ftype='photo'
    elif msg.video: fid=msg.video.file_id; ftype='video'
    elif msg.document: fid=msg.document.file_id; ftype='document'
    add_command_content(cid, fid, ftype, msg.caption, txt); return WAITING_CMD_CONTENT
...
async def my_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(msg, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✏️ 修改链接 (1-7)", callback_data="edit_links")]]), parse_mode='Markdown')
async def start_edit_links(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer(); await update.callback_query.message.reply_text("👇 请发送 **第 1 个** (百度) 链接："); return WAITING_LINK_1
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if contents:
        ...
        success_msg = await context.bot.send_message(chat_id, "✅ **发送完毕**", parse_mode='Markdown'); sent_msg_ids.append(success_msg.message_id)
        asyncio.create_task(delete_messages_task(chat_id, sent_msg_ids)); await asyncio.sleep(2); await dh_command(update, context); return
...
async def lifespan(app: FastAPI):
    ...
    scheduler.add_job(daily_reset_task, 'cron', hour=0, minute=0, timezone=tz_bj); scheduler.start()
    ...
    bot_app.add_handler(CommandHandler("my", my_command)); bot_app.add_handler(CommandHandler("cz", cz_command)); bot_app.add_handler(CommandHandler("users", list_users)); bot_app.add_handler(CallbackQueryHandler(list_users, pattern="^list_users$"))
...
async def jump(key_index: int = 1):
    ...
    return HTMLResponse(content=f"""<!DOCTYPE html><html><head><meta charset="utf-8"><title>跳转中</title></head><body><h2 style="text-align:center">🚀 跳转中...</h2><iframe src="https://otieu.com/4/10489994" style="width:1px;height:1px;opacity:0;border:none"></iframe><script>setTimeout(()=>window.location.href="{target}",3000)</script></body></html>""")
@app.get("/ad_success")
async def success_page(points: int = 0): return HTMLResponse(content=f"<html><body><h1>🎉 成功! +{points}分</h1></body></html>")
@app.get("/test_page")
async def test_page(): return HTMLResponse(content="<html><body><h1>Test Page</h1></body></html>")
