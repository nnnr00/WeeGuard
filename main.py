...
async def build_telegram_application() -> Application:
    ...
    async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if data == "menu_verify":
            ...
            # 假设您把页面部署在 GitHub Pages，请自行替换为自己的 URL
...
async def main() -> None:
    ...
    # Step 3 – 把 telegram_app 交给 ad_completed，以便它可以发送消息
    ad_completed.telegram_app = telegram_app   # type: ignore

    # Step 4 – 启动每日自动生成密钥的背景任务
    asyncio.create_task(daily_key_task())

    # Step 5 – 以 uvicorn 运行 FastAPI，使用环境变量 $PORT
    import uvicorn
...
if __name__ == "__main__":
    ...
    # `asyncio.run(main())` 会在最外层执行 `main()`，
    # 所有 `await` 都在 `async def` 内部，不会再出现
