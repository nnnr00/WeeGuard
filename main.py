...
def get_u(uid):
    ...
    if not u:
        ...
        conn.commit(); cur.execute("SELECT * FROM users WHERE user_id = %s", (uid,)); u = cur.fetchone()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    v_locked = u[4] and datetime.now() < u[4]
    txt = "👋 欢迎使用【VIP服务机器人】\n━━━━━━━━━━━━━\n💎 验证身份入群\n🪙 签到领分兑换商品\n📦 获取私密资源包"
    kbd = [
        [InlineKeyboardButton("💎 身份验证" if not v_locked else "🔒 锁定中", callback_data="v_start" if not v_locked else "none")],
    ...
    if ID_WELCOME.startswith("这里"):
        ...
        await msg.reply_text(txt, reply_markup=InlineKeyboardMarkup(kbd))
    else:
        msg = update.callback_query.message if update.callback_query else update.message
        await msg.reply_photo(ID_WELCOME, caption=txt, reply_markup=InlineKeyboardMarkup(kbd))
...
async def jf_sign(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; u = get_u(q.from_user.id); today = datetime.now().date()
    if u[2] == today: await q.answer("❌ 今日已签到", show_alert=True); return
    r = random.randint(3, 8); conn = psycopg2.connect(DB_URL); cur = conn.cursor()
    cur.execute("UPDATE users SET points = points + %s, last_checkin = %s WHERE user_id = %s", (r, today, u[0]))
    conn.commit(); cur.close(); conn.close()
    log_h(u[0], "每日签到", f"+{r}"); await q.answer(f"🎉 获得 {r} 积分！", show_alert=True); await jf_ui(update, context)

async def jf_pay_ui(update: Update, context: ContextTypes.DEFAULT_TYPE):
    u = get_u(update.effective_user.id)
    w_l = u[5] or (u[7] and datetime.now() < u[7])
    a_l = u[8] or (u[10] and datetime.now() < u[10])
    txt = "💳 **充值中心 (5元=100分)**\n⚠️ 微信/支付宝各限一次，请勿重复充值。"
    kbd = [[InlineKeyboardButton("💹 微信支付" if not w_l else "❌ 已用/锁定", callback_data="p_wx" if not w_l else "none")],
           [InlineKeyboardButton("💹 支付宝" if not a_l else "❌ 已用/锁定", callback_data="p_ali" if not a_l else "none")],
...
async def on_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "home": await start(update, context)
        ...
        u = get_u(q.from_user.id); conn = psycopg2.connect(DB_URL); cur = conn.cursor()
        cur.execute("SELECT action, amount, ts FROM history WHERE user_id = %s ORDER BY ts DESC LIMIT 10", (u[0],))
        rows = cur.fetchall(); cur.close(); conn.close()
        lt = "\n".join([f"• `{r[2].strftime('%m-%d')}` {r[0]} ({r[1]})" for r in rows])
        await q.edit_message_text(f"💰 余额：{u[1]}\n\n最近记录：\n{lt or '暂无'}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 返回", callback_data="j_page")]]), parse_mode="Markdown")
    elif d == "v_start":
        ...
        pid = int(d.split("_")[1]); conn = psycopg2.connect(DB_URL); cur = conn.cursor()
        cur.execute("SELECT 1 FROM redemptions WHERE user_id = %s AND p_id = %s", (q.from_user.id, pid))
        if cur.fetchone():
        ...
        if pid == -1:
            cur.execute("INSERT INTO redemptions (user_id, p_id) VALUES (%s, %s)", (q.from_user.id, -1))
        ...
        b = context.user_data.get('tmp_b'); u = get_u(q.from_user.id)
        if u[1] < b['price']: await q.answer("❌ 余额不足", show_alert=True); await dh_ui(update, context); return
        conn = psycopg2.connect(DB_URL); cur = conn.cursor()
        cur.execute("UPDATE users SET points = points - %s WHERE user_id = %s", (b['price'], u[0]))
        cur.execute("INSERT INTO redemptions (user_id, p_id) VALUES (%s, %s)", (u[0], b['id']))
        log_h(u[0], f"兑换:{b['name']}", f"-{b['price']}"); conn.commit(); cur.close(); conn.close()
...
if __name__ == '__main__':
    ...
    v_conv = ConversationHandler(
        ...
        states={
            ...
            P_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, lambda u,c: [c.user_data.update({'pn':u.message.text}), "输入价格："][1] and P_PRICE)],
            P_PRICE: [MessageHandler(filters.TEXT & ~filters.COMMAND, lambda u,c: [c.user_data.update({'pp':u.message.text}), "发送内容："][1] and P_CONT)],
            P_CONT: [MessageHandler(filters.ALL & ~filters.COMMAND, lambda u,c: [psycopg2.connect(DB_URL).cursor().execute("INSERT INTO products (name,price,ptype,fid,txt) VALUES (%s,%s,%s,%s,%s)", (c.user_data['pn'], c.user_data['pp'], 'photo' if u.message.photo else 'text', u.message.photo[-1].file_id if u.message.photo else None, u.message.text or u.message.caption)), "✅ 上架成功"][1] and ConversationHandler.END)],
            ...
            L_CMD: [MessageHandler(filters.TEXT & ~filters.COMMAND, lambda u,c: [c.user_data.update({'lc':u.message.text, 'lm':[]}), "发送内容，完成后点结束", InlineKeyboardMarkup([[InlineKeyboardButton("结束", callback_data="l_fin")]])][1] and L_CONT)],
            L_CONT: [CallbackQueryHandler(lambda u,c: [psycopg2.connect(DB_URL).cursor().execute("INSERT INTO forward_lib (cmd_text,messages) VALUES (%s,%s)", (c.user_data['lc'], Json(c.user_data['lm']))), "✅ 保存成功"][1] and ConversationHandler.END, pattern="^l_fin$"), MessageHandler(filters.ALL & ~filters.COMMAND, lambda u,c: c.user_data['lm'].append({'cid':u.message.chat_id, 'mid':u.message.message_id}))]
        },
