...
async def daily_reset_task():
    """每日0点重置次数 (静默) / 每周一重置密钥"""
    # 这里的逻辑主要由数据库日期字段控制，无需复杂操作
    # 密钥重置由 weekly_reset_task 控制
    pass
...
async def check_vip_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt.startswith("4768"):
        ...
        new_fails = update_fail(user.id, 'vip_buy', fails, 10)
...
# --- Admin Handlers (其他) ---
# 此处省略部分重复的 Admin handlers (cmd, prod, upload)，为保证 NameError 修复，
# 它们已在之前的完整代码中提供。若需要，我可以再次完整提供所有 Admin 子函数。
# 为确保 NameError 彻底解决，请确保 `admin_entry` 已经在上面定义了。
# 下面补充其他必要的 Admin 子函数，确保引用完整。
...
async def list_admin_prods(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if nav: kb.append(nav); kb.append([InlineKeyboardButton("🔙 返回", callback_data="manage_products_entry")])
...
async def list_cmds(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if nav: kb.append(nav); kb.append([InlineKeyboardButton("🔙 返回", callback_data="manage_cmds_entry")])
...
async def lifespan(app: FastAPI):
    ...
    bot_app.add_handler(CallbackQueryHandler(checkin_handler, pattern="^do_checkin$")); bot_app.add_handler(CallbackQueryHandler(quark_key_btn_handler, pattern="^get_quark_key$")); bot_app.add_handler(CallbackQueryHandler(get_quark_key_entry, pattern="^get_quark_key_v7$"))
