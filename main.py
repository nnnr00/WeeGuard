...
async def dh_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id; is_unlocked = is_exchange_unlocked(user_id)
    if not is_unlocked:
    ...
    offset = 0
    if update.callback_query and "list_prod_" in update.callback_query.data: offset = int(update.callback_query.data.split("_")[-1])
    rows, total = get_products_list(limit=10, offset=offset)
    is_v, _ = is_vip(user_id); daily_used, has_free = check_daily_free(user_id); kb = []
...
async def exchange_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "do_buy_" in data:
        ...
        record_purchase(uid, pid); await query.message.reply_text(f"🎉 **兑换成功！**\n\n📦 **内容：**\n`{prod[3] or ''}`", parse_mode='Markdown')
        if prod[4]:
            try:
                if prod[5]=='photo': await context.bot.send_photo(uid, prod[4])
                elif prod[5]=='video': await context.bot.send_video(uid, prod[4])
...
async def receive_prod_content(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ **商品上架成功！**", reply_markup=kb, parse_mode='Markdown'); return ConversationHandler.END
...
async def receive_cmd_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ `{name}` 创建。\n👇 发送内容 (多条)，完成后点按钮。", reply_markup=kb, parse_mode='Markdown'); return WAITING_CMD_CONTENT
...
async def finish_cmd_bind(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer(); kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 返回", callback_data="manage_cmds_entry")]])
    await query.edit_message_text("🎉 绑定完成！", reply_markup=kb); return ConversationHandler.END
...
async def my_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = f"👮‍♂️ **密钥管理** ({info[-1]})\n\n"
    for i in range(1, 8): msg += f"🔑 Key{i}: `{info[k_idx=(i-1)*2+1]}`\n🔗 Link{i}: {info[(i-1)*2+2] or '❌'}\n\n"
...
async def view_files_flow(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(q.message.chat_id, "--- END ---", reply_markup=kb); return ConversationHandler.END
...
async def execute_delete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer(); did = q.data.split('_')[-1]; delete_file_by_id(did)
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 返回", callback_data="back_to_admin")]]); await q.delete_message(); await context.bot.send_message(q.message.chat_id, "已删除", reply_markup=kb)
            ...
            if len(media_group) == len(chunk) and len(media_group) > 1:
                ...
                for item in chunk:
                    try:
                        ...
                        elif item[2] == 'photo': m = await context.bot.send_photo(chat_id, item[1]) # No caption
